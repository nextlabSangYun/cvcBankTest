		...
		<div class="con_col2">
			...
			<div class="r_cont">
				...
				<div class="btn_wrap" v-show="!(isMod || isAdd)">
					...
					<button v-show="selected.lvl < 3" class="btn_default" v-on:click="setAdd">하위메뉴추가</button>
					<!-- 현재 선택된 노드가 0000인 가상의 최상위 카테고리인 경우 이름 변경 및 삭제 안됨 -->
					<button v-show="selected.lvl > 0" class="btn_default" v-on:click="setMod" >카테고리명 변경</button>			
					<button v-show="selected.lvl > 0" class="btn_default" v-on:click="delCategory">삭제</button>			
				</div>
				<div v-show="isMod" class="btn_wrap">
					<button class="btn_default" v-on:click="modCategory">적용</button>
					<button class="btn_cancel" v-on:click="isMod=false;isAdd=false;">닫기</button>
				</div>
				<div v-show="isAdd" class="btn_wrap">
					<button class="btn_default" v-on:click="addCategory">적용</button>
					<button class="btn_cancel" v-on:click="isMod=false;isAdd=false;">닫기</button>
				</div>
			</div>
		</div>
...
var vueApp = new Vue({
	...
	methods:{
		...
		, delCategory:function(){
			if(this.selected.child != undefined && this.selected.child > 0) {
