			...
			<div class="g_table04">
				<table>
					...
					<tbody>
						...
						<tr>
							<th>개선 및 수정사항</th>
							<td><div class="txt" v-html="projectView.improveField"></div></td>
						</tr>
						<tr>
							<th>비고</th>
				...
				<table>
					...
					<tbody>
						<tr>
							<th>테스트측정모델</th>
							<td>
								<button v-for="(list, index) in projectBmtModelList" class="tb_btn03_home" @click="goShowPopupLayer('/bmt/project/projectModelHstList?modelSeq='+list.modelSeq); return false;">{{ list.modelNm }}({{ list.modelVer }})</button>
							</td>
						</tr>
						<tr>
							<th>비교측정모델</th>
							<td>
								<button v-for="(list, index) in projectBmtCompareModelList" class="tb_btn02_home" @click="goShowPopupLayer('/bmt/project/projectModelHstList?modelSeq='+list.modelSeq); return false;">{{ list.modelNm }}({{ list.modelVer }})</button>
							</td>
						</tr>
						<tr>
							<th>현재 연결구성</th>
							<td :class="{has_btn:projectView.nowJoinCompose}">
								<button v-if="projectView.nowJoinCompose" class="tb_btn02" @click="goShowPopupLayer('/bmt/project/joinCompHstList?pid='+pid); return false;">이력 보기</button>
		...
		, methods: {
			getProjectView: function() {
				$.get("/bmt/project/getProjectEditInfoView?pid="+cont.pid, function(data){
					...
					if(cont.projectView.improveField != null){
						cont.projectView.improveField = cont.projectView.improveField.replace(/(?:\n)/g, '<br>');
						cont.projectView.improveField.split('\n').join('<br>');
					}
					...
					cont.projectBmtCompareModelList = data.projectBmtCompareModelList;
				...
				$.post("/bmt/project/getIssueCateDataList", this.$data, function(data){
					if (!isEmpty(data.issueCateCntList)) {
						data.issueCateCntList.forEach(function(e){
							obj.data.push(e.cdCount);
							obj.categories.push(e.codeNm);
							obj.bigCateId.push(e.issueLcateCd);
						});
					}
...
/* 			, processResponse: function(response) {
			...
			, getIssueChartView: function() {
				...
				for (var i = 0; i < cont.issueCntList.length; i++){
					...
					fwCntList.push(cont.issueCntList[i].fwCnt);
					heCntList.push(cont.issueCntList[i].heCnt);
					normalCntList.push(cont.issueCntList[i].normalCnt);
				}
				chartId = $("#issueChart");
				
				series = [
					{name:"전체", color:"#596e82", data: totCntList}
					, {name:"오픈", color:"#ff7473", data: openCntList}
					, {name:"H/E이슈", color:"#f99e53", data: heCntList}
					, {name:"차기F/W적용", color:"#f4e134", data: fwCntList}
					, {name:"정상시나리오", color:"#8be53c", data: normalCntList}
					, {name:"수정중", color:"#41c4d6", data: ingCntList}
				...
				chartId.kendoChart({
					...
					, seriesClick: function(e) {
						//statCheck 0전체, 1오픈, 2H/E이슈, 3차기F/W적용, 4정상시나리오, 5수정중, 6수정완료
			...
			, getIssueCateChartView: function(obj) {
				$("#issueCateChart").kendoChart({
					...
					, seriesClick: function(e) {
						var issueLcateCd = obj.bigCateId[e.point.categoryIx];
						if(isEmpty(issueLcateCd)){
							issueLcateCd = "";
						}
						location.href = "/bmt/issue/issueList?pid="+cont.pid+"&issueLcateCd="+issueLcateCd;
					}
					, valueAxis: {
						line: {
							visible: false
