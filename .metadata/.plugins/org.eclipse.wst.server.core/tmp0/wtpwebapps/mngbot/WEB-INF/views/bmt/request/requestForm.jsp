...
<form name="editForm" method="post" enctype="multipart/form-data" onsubmit="return false;">
				...
				<tbody>
					...
					<tr>
						...
						<td class="notice_txt" v-else>
							<div class="txt" v-html="view.reqCont" style="height: 400px;">
							</div>
						</td>
					</tr>
					<tr>
						<th>기간</th>
						<td v-if="(reqId === '') || (reqId !== '' && idChk && (view.progressStat == 'D201' || view.progressStat == 'D202'))">
							<input type="text" class="input_date" id="reqStDt" name="reqStDt" v-model="view.reqStDt" readonly>
							<span class="bar">~</span>
							<input type="text" class="input_date" id="reqEdDt" name="reqEdDt" v-model="view.reqEdDt" readonly>
						</td>
						<td v-else-if="view.reqStDt !== '' && view.reqStDt !== null">
							{{ view.reqStDt }} ~ {{ view.reqEdDt }}
						</td>
						<td v-else>
						</td>
					</tr>
					...
					<tr>
						...
						<td class="reg" v-if="(reqId === '') || (reqId !== '' && idChk && (view.progressStat == 'D201' || view.progressStat == 'D202'))">
							<div class="member">
								<c:forEach items="${mediaUsers}" var="user">
									<span>
										<input type="checkbox" id="${user.userId}" name="mediaUserCheck"><label for="${user.userId}">${user.userNm}</label>
									</span>
								</c:forEach>
							</div>
							<div class="reg_list" v-show="view.managerList.length>0">
								<ul>
									<li v-for="(p, index) in view.managerList"><span>{{p.userNm}}({{p.userGroupNm}})</span><button class="del" @click="delManagerList(index,p.managerId)"></button></li>
								</ul>
							</div>
						</td>
						<td class="reg" v-else>
	...
	<div class="btn_wrap">
		...
		<button class="g_btn03" v-if="reqId !== '' && isMng && (view.progressStat == 'D201' || view.progressStat == 'D202')" @click="goShowPopupLayer('/bmt/request/requestCancel?reqId=' + requestForm.reqId); return false;">반려</button>
...
var requestForm = new Vue(
		{
			...
			mounted : function() {
				...
				$('[name="mediaUserCheck"]').on('click', function(){
					var userId= $(this).attr('id');
					var userNm = $(this).next().text();
					var checkVal = $(this).prop('checked');
					var managerSeq = -1;
					
					for (var i=0; i<requestForm.view.managerList.length; i++) {
						var manager = requestForm.view.managerList[i];
					...
					if (checkVal) {
						if (managerSeq < 0) {
							requestForm.view.managerList.push({
								managerId : userId,
								userNm : userNm,
								userGroupNm : 'SKB 미디어솔루션팀'
							});
						}
					} else {
						requestForm.view.managerList.splice(managerSeq, 1);
					}
				});
			},
			updated : function() {
			...
			methods : {
				setContents: function() {
					...
					$(".input_date").on("change", function() {
						requestForm['view'][this.name] = $(this).val();
					});
					
					var view = this.view;
					$('[name="mediaUserCheck"]').each(function() {
						var userId = $(this).attr('id');
						for (var i=0; i<view.managerList.length; i++) {
							var manager = view.managerList[i];
							if (manager.managerId == userId) {
				...
				, delManagerList: function(index, delUserId) {
				...
				, btnSave: function(){
					...
					if (requestForm.view.reqStDt != "" || requestForm.view.reqEdDt != "") {
						if (requestForm.view.reqStDt == "") {
							alert("BMT 요청 시작일자를 입력하세요.");
							return false;
						}
						if (requestForm.view.reqEdDt == "") {
							alert("BMT 요청 종료일자를 입력하세요.");
							return false;
						}
						if (requestForm.view.reqStDt > requestForm.view.reqEdDt) {
