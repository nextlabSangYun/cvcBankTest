...
public class QaTcController extends BaseController {
	 ...
	 * @return
	 */
	@RequestMapping("/qa/tc/getlcateIdList")
	public View getTcList(Model model, String pid){
		model.addAttribute("cateList",qaTcService.selectCateList(pid));
		return new MappingJackson2JsonView();
	}
	 ...
	 * @return
	 */
	@RequestMapping("/qa/tc/getTcExcel")
	public ExcelDownloadView getIssueExcel(Model model,QaTcCriteria criteria, HttpServletResponse response) {
		List<QaTcInfotVo> list= qaTcService.selectTcList(criteria);
		ExcelHeader[] headers = new ExcelHeader[]{
				new ExcelHeader("tcDiviCd", "구분", "", "B900", HorizontalAlignment.CENTER)
				, new ExcelHeader("tcId", "TC No.", HorizontalAlignment.CENTER)
				...
				, new ExcelHeader("tcNm", "Test Case", HorizontalAlignment.LEFT )
				, new ExcelHeader("lastResultCd", "최근 테스트 결과", "", "B300", HorizontalAlignment.CENTER)
		};
