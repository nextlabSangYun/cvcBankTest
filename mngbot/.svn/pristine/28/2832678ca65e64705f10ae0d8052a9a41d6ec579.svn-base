				...
				<table>
					...
					<tbody>
						<tr v-for="(listIssue, idx) in pagingList">
							...
							<td class="l_align">
								<span v-for="modelList in listIssue.modelInfoList">
									{{modelList}}
								</span>
							</td>
							...
							<td><button v-if="listIssue.isYn" class="file" @click="showPopupLayer('/bmt/issue/issueAttachList?issueId=' + listIssue.issueId); return false;"></button></td>
