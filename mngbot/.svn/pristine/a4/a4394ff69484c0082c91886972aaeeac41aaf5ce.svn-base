...
<div class="content">
	<div class="sub_layout has_snb" v-if="isCheck">
		<div class="tit_wrap">
			<p class="cate_tit">투표 설정</p>
			<div class="btn_wrap">
				<button class="g_btn03" @click="btnSave">저장</button>
			</div>
		</div>
		<div class="g_table04">
			<table>
				...
				<tbody>
					...
					<tr>
						<th>투표제목<strong>*</strong></th>
						<td class="reg insert_con">
							<input class="full" type="text" v-model.trim="decisionMaking.voteSubject">
						</td>
					</tr>
					<tr>
						<th>투표내용<strong>*</strong></th>
						<td class="reg insert_con">
							<textarea class="full" cols="30" rows="5" v-model.trim="decisionMaking.voteCont" placeholder="투표 내용을 입력하세요."></textarea>
						</td>
					</tr>
					<tr>
						<th>투표대상<strong>*</strong></th>
						<td class="reg insert_con">
							<div class="sel_sort">
								<user-select ref="userSelect" :add-fn="addVoterList"></user-select>
							</div>
							<div class="reg_list" v-show="decisionMaking.voterList.length>0">
								<ul>
									<li v-for="(p, index) in decisionMaking.voterList"><span>{{p.userNm}}({{p.userGroupNm}})</span><button class="del" @click="delVoterList(index)"></button></li>
								</ul>
							</div>
						</td>
					</tr>
					<tr>
						...
						<td class="insert_con">
							<select v-model="decisionMaking.voteStat">
								<option value="">- 선택 -</option>
								<option v-for="list in getVoteStatCodeList" :value="list.codeId">{{list.codeNm}}</option>
							</select>
						</td>
					</tr>
					<tr v-if="decisionMaking.voteStat == 'C304'">
						<th>최종결과<strong>*</strong></th>
						<td class="insert_con">
							<select v-model="decisionMaking.lastResultCd">
								<option value="">- 선택 -</option>
					...
					<tr v-if="decisionMaking.voteStat == 'C304'">
						...
						<td class="reg insert_con">
							<textarea class="full" cols="30" rows="5" v-model.trim="decisionMaking.voteGeneralReview" placeholder="총평을 입력하세요."></textarea>
						</td>
					</tr>
					<tr v-else>
						<th>총평<strong>*</strong></th>
						<td class="reg insert_con">
							<textarea class="full" cols="30" rows="5" disabled="disabled" placeholder="총평을 입력하세요."></textarea>
						</td>
					</tr>
				</tbody>
			</table>
		...
		<div class="tit_wrap mgtop40">
			...
			<div class="btn_wrap">
				<button class="g_btn04" @click="issueExcelDown"><span>다운로드</span></button>
				<button class="g_btn01" @click="addIssueLargeAdd">이슈 추가</button>
			</div>
		</div>
		<div class="issue_list" v-for="(issue, index) in decisionMaking.issueList">
			<div class="list_title">
				<p># {{index + 1}}</p>
				<button @click="btnIssueDel(issue.issueId)"></button>
			</div>
				...
				<table>
					...
					<tbody>
						<tr v-for="(management, index) in issue.voteManagementList">
							...
							<td>{{management.modDtm | dateTimeFormat}}</td>
						</tr>
		...
		, methods: {
			getVote: function() {
				$.get("/bmt/issue/getVote?pid="+ pid, function(data){
					...
					} else if (view.decisionMaking.voteStat == 'C304') {
						for (var i=0; i<view.decisionMaking.issueList.length; i++) {
							var issue = view.decisionMaking.issueList[i];
							if (issue.voteManagementList.length > 0) {
								for (var j=0; j<issue.voteManagementList.length; j++) {
			...
			, issueExcelDown: function() {
				var param = {
					pid : this.pid
				};
				excelFileDown('/bmt/issue/getVoteMngListExcel', param);
			}
		}
	});
</script>