...
public abstract class ExcelParser<E extends ExcelResult> {
		...
		for (Field field : fields) {
			...
			if (exceptionMessage.isEmpty()) {
				...
				if (codeValid != null && codeValid.codeType() != null && value != null) {
					// 코드 정보 조회
					CodeVo code = codeComponent.getCodeByCodeValue(codeValid.codeType(), value.toString());
					if (code == null) {
						if (value == null || value.equals("")) {
							if (log.isDebugEnabled()) {
	 ...
	 * @return
	 */
	public boolean isFileVoValidExcelFile() {
		try {
			if (!org.springframework.util.StringUtils.endsWithIgnoreCase(this.fileVo.getFileNm(), "xlsx")) {
				this.fileVo.setMessage("엑셀 파일이 아닙니다.");
				return false;
			}

			if (!this.isFileVoFormalExcelForm()) {
				this.fileVo.setMessage("잘못된 엑셀 파일 형식입니다.");
				return false;
			}

			if (!this.isFileVoInMaxInsertCount()) {
				this.fileVo.setMessage("등록 가능한 갯수를 초과했습니다.");
				return false;
			}
	 ...
	 * @return
	 */
	private boolean isFileVoFormalExcelForm() {
		try {
			Workbook workbook = WorkbookFactory.create(new FileInputStream(this.fileVo.getFileFullPath()));
			Sheet sheet = workbook.getSheetAt(0);
			Row row = sheet.getRow(this.getHeaderStartRow());
	 ...
	 * @return
	 */
	private boolean isFileVoInMaxInsertCount() {
		try {
			Workbook workbook = WorkbookFactory.create(new FileInputStream(this.fileVo.getFileFullPath()));
			Sheet sheet = workbook.getSheetAt(0);
	 ...
	 * @return
	 */
	public List<E> getFileVoDataList() {
		List<E> list = new ArrayList<E>();
		try {
			Workbook workbook = WorkbookFactory.create(new FileInputStream(this.fileVo.getFileFullPath()));
			Sheet sheet = workbook.getSheetAt(0);
