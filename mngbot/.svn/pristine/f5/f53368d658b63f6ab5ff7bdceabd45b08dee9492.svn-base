				...
				<table>
					...
					<tbody>
						...
						<tr>
							<th>Test Case 이름</th>
							<td v-if="prjVo.projectStat != 'C304'">
							<textarea class="full rows2" v-model="tc.tc"></textarea></td>
							<td v-else><div class="txt" v-html="tc.tc"></div></td>
						</tr>
					</tbody>
				</table>
			</div>
			...
			<div class="g_table02">
				<table>
					...
					<tbody>
						<tr>
							<th>비고</th>
							<td><textarea v-model="tc.bigo" class="full rows2"></textarea></td>
						</tr>
						<tr>
							<th>파일첨부</th>
							<td>
			...
			methods : {
				setup : function() {
					...
					$.post("/bmt/tc/getTc", this.tcCriteria, function(data) {
						vueApp.tc = data.tc;
						if(vueApp.prjVo.projectStat == 'C304'){
							vueApp.tc.tc = vueApp.tc.tc.replace(/(?:\n)/g, '<br>');
							vueApp.tc.tc.split('\n').join('<br>');	
						}
