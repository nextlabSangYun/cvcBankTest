	...
	<div class="sub_layout has_snb">
		<div class="cont_wrap">
			<div class="tb_desc">
				...
				<div class="btn_wrap">
					<button v-if="prjVo.projectStat != 'C304' " class="g_btn01" @click="goShowPopupLayer('/bmt/board/boardForm?pid=' + '<c:out value="${param.pid}"/>'); return false;">새글쓰기</button>
				</div>
			</div>
			<div class="g_table">
				<table>
					...
					<tbody>
						<tr v-for="(list,idx) in pagingList">
							...
							<td class="l_align"><p class="ellips"><a href="#" class="link" @click="showPopupLayer('/bmt/board/boardForm?boardSeq=' + list.boardSeq + '&pid=' + list.pid); return false;">{{ list.subject}}</a></p></td>
							<td>{{ list.userNm }}({{ list.userGroupNm }})</td>
							<td><button v-if="list.isYn" class="file" @click="goShowPopupLayer('/bmt/board/boardAttachList?boardSeq=' + list.boardSeq); return false;"></button></td>
	...
	var boardList = new Vue({
		...
		, methods: {
			getBoardList: function(paramPid){
				$.get("/bmt/board/getBoardList?pid=" + paramPid, function(data){
			...
			, getFileDown: function(idx) {
				fileDown(boardList.boardList[idx].filePath, boardList.boardList[idx].fileNm, boardList.boardList[idx].orgFileNm);
			}
		}
	})
</script>