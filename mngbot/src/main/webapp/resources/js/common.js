$(function() {
	...
	$(".onlyAlpha").keyup(onlyAlpha);
	$(".onlyAlphaNum").keyup(onlyAlphaNum);
...
function voteStateHeight() {
	// bmt 이슈의사결정 투표현황 위치조절
	$(".mirror_height").each(function() {
		var tHeight = $(this).parent().find('.target_height').height();
		var mHeight = $(this).height();
		$(this).css("margin-top", (tHeight-mHeight-13)/2);
	});
}
...
//			}
    		$(".popLayer").css("top", $(".popLayer").css('top'));
			if ($(".popLayer").hasClass('hded')) {
    			$(".popLayer").width(pWidth).css({"margin-left": "0"});
    		} else {
...
/***********************************************************************
 | Hded 적용
 ***********************************************************************/
 function hdedshowPopupLayer(popSrc, popUp){
	top._hdedshowPopupLayer(popSrc, popUp);
}

var POPUP_INIT_ID = 0;
var _popupLayerID = POPUP_INIT_ID;

function _hdedshowPopupLayer(popSrc, popUp) {
	var popSrcUrl = popSrc;
	if(popSrcUrl.indexOf("?") > 0){
		popSrcUrl += '&win=modal'
	}else {
	...
	$(".popLayer").show();
	// hded용 클래스 추가
	$('.popLayer').addClass('hded');
	$('.pop_wrap').addClass('hded');
	$('.ly_close').addClass('hded');
	$('body').css({"background": "#fff"});
}

function hdedsetIssuePopup(obj, reset){
	// 가로
	var pWidth = $(".pop_wrap").width();
	parent.$(".popLayer").width(pWidth);
	// 세로
	var pHeight = $(".pop_wrap").height();
	parent.$(".popLayer , #iframePopLayer").add($(".pop_cont")).height("100%");
	parent.$(".popLayer , #iframePopLayer").height(pHeight);
	parent.$(".popLayer").css({"top" : parent.window.pageYOffset + 50});
	// hded용 클래스 추가
	$('.popLayer').addClass('hded');
	...
	$('.ly_close').addClass('hded');
	$('body').css({"background": "#fff"});
	parent.$(".popLayer").css({"opacity": "1"});
}

function hdedsetPopup(obj, reset){
	// 가로
	var pWidth = $(".pop_wrap").width();
	parent.$(".popLayer").width(pWidth);
	// 세로
...
function hdedgoShowPopupLayer(url) {
	if (url) {
		hdedshowPopupLayer(url);
		location.href = '#';
	}
}
/***********************************************************************/
...
function modalLayer(){
	var $btn_modal = null;
	var $btn_modal_close = null;
	// var $modal_wrap = null;
	var selected_modal = null;
	function init(){
		$btn_modal = $(".modal_open");
		$btn_modal_close = $(".modal_close");
		// $modal_wrap = $(".modal_background");
	}
	function initEvent(){
//		$btn_modal.on("click", function(){
//			selected_modal = $(this).data("modal");
//			$("[data-modal-target="+selected_modal+"]").addClass("on");
//		});
//		$btn_modal_close.on("click", function(){
//			$("[data-modal-target="+selected_modal+"]").removeClass("on");
//		});
...
function modelOpen(id) {
	$("[data-modal-target="+id+"]").addClass("on");
}

function modleClose(id) {
	$("[data-modal-target="+id+"]").removeClass("on");
}
