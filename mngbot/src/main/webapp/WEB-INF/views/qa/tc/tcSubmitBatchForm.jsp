...
<div class="pop_wrap" style="width:610px;" id="vueApp">
	...
	<div class="btn_wrap">
		<button class="btn_proc" @click="submit">등록</button>
...
<script>
	var cont = new Vue({
		...
		, methods: {
			getTcList: function() {
				this.param.tcResultCd = parent.list.qaResult;
				...
				$.post('/qa/tc/getTcList', param, function(data){
			...
			, submit: function() {
				...
				for(var key in this.param.xqmsArray){
					if(isEmpty(this.param.xqmsArray[key].mac)){
						alert("맥 어드레스를 - 기호 제외 12문자로 입력해주세요.");
						return;
					}
					if(this.param.xqmsArray[key].mac.length != 12){
						alert("맥 어드레스를 - 기호 제외 12문자로 입력해주세요.");
						return;
					}
				}
