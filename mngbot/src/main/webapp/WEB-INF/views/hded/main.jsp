...
<div class="content">
	<div class="sub_layout">
		...
		<div class="btm_con con_col2">
			...
			<div class="r_cont">
				<p class="cate_tit prj_tc_title">TC 변경이력(최근 30건)
					<button class="btn_default" @click="goTcChangeHist">
						<span>H/E TC 변경이력 전체 조회</span>
					</button> 
				</p>
				<div class="prj_tc_frame">
					<div class="tc_title">
						<div class="g_table">
							<table>
							...
							<table>
								...
								<tbody>
									<tr v-for="list in tcChangeHistList">
										...
										<td class="prj_title">{{ list.projectNm }}</td>
										<td class="prj_title">{{ list.tcNm }}</td>
										<td>
											<div class="tb_fixed_btns">
												<button class="smbtn_default" @click="goTcChangeHistByGroupId(list.tcGroupId)">변경이력</button>
											</div>
										</td>
									</tr>
									<tr v-if="tcChangeHistList.length == 0">
										<td colspan="4">변경된 테스트케이스가 없습니다.</td>
									</tr>
								</tbody>
							</table>
						</div>
		...
		<div class="top_fixed_btns">
			<div class="btn_link_all" v-for="m in getHdedMenuList" >
				<button v-if="m.pageType == 'C103'" @click="goShowPopupLayer(m.menuUrl)"><img :src="'/images/hded/btn_' + m.menuId + '.png'" :alt="m.menuNm"></button>
				<button v-else-if="m.pageType == 'C102'" @click="goPopupView(m.menuUrl, m.menuNm)"><img :src="'/images/hded/btn_' + m.menuId + '.png'" :alt="m.menuNm"></button>
...
<script>
	var mainView = new Vue({
		...
		, methods: {
			...
			, goPopupView: function(url, menuNm) {
				if (url) {
