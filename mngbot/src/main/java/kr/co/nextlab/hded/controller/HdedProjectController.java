...
public class HdedProjectController extends BaseController {
	...
	public View getHeProjectInfoView(Model model, String pid){
		...
		// 프로젝트 차수 정보
		model.addAttribute("heProjectChasuHstList", hdedProjectService.selectHeProjectChasuHstList(pid));
		return new MappingJackson2JsonView();
	}
	...
	@RequestMapping("/hded/project/modHeProjectUseYnPrc")
	public View modHeProjectUseYnPrc(Model model, @RequestBody HeProjectVo heProjectvo){
		heProjectvo.setModId(super.getUser().getUserId());
		model.addAttribute("save", hdedProjectService.updateHeProjectUseYn(heProjectvo));
		return new MappingJackson2JsonView();
	}
	
	/**
	 * HE 프로젝트 tc pass rate 조회
	 * @param model
	 * @param vo
	 */
	@RequestMapping("/hded/project/getTcPassRate")
	public View getTcPassRate(Model model, String pid) {
		model.addAttribute("tcPassRateList", hdedProjectService.selectTcPassRate(pid));
		return new MappingJackson2JsonView();
	}
	 ...
	 * @return
	 */
	@RequestMapping("/hded/project/getSystemNmList")
	public View getSystemNmList(Model model, String systemNm) {
		model.addAttribute("systemNmList", hdedProjectService.selectSystemNmList(systemNm));
		return new MappingJackson2JsonView();
	}
	 ...
	 * @return
	 */
	@RequestMapping("/hded/project/modHeProjectRestartPrc")
	public View modHeProjectRestartPrc(Model model, @RequestBody HeProjectVo heProjectVo){
		
		boolean save = false;
		try {
			heProjectVo.setProjectStat("C301");
			heProjectVo.setProjectResultCd("D101");
			heProjectVo.setModId(super.getUser().getUserId());
			save = hdedProjectService.updateRestartHeProject(heProjectVo);
