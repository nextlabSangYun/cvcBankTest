...
public class HdedTcController extends BaseController {
	 ...
	 * @param model
	 */
	@RequestMapping("/hded/tc/getSystemIdBucketList")
	public View systemIdBucketList(Model model, @RequestBody HeTcTmplCriteria criteria) {
		List<HeTcTmplVo> bucketList = hdedTcService.selectSystemIdBucketList(criteria);
		model.addAttribute("bucketList", bucketList);
		return new MappingJackson2JsonView();
	}
	
	/**
	 * 버킷리스트 리스트 excel down
	 * @param model
	 */
	@RequestMapping("/hded/tc/getExcelBucketList")
	public ExcelDownloadView getExcelBucketList(Model model, HeTcTmplCriteria criteria) {
		List<HeTcTmplVo> list = hdedTcService.selectBucketList(criteria);

		ExcelHeader[] headers = new ExcelHeader[] {
			new ExcelHeader("systemNm", "시스템", HorizontalAlignment.CENTER)
			, new ExcelHeader("lcateNm", "대분류", HorizontalAlignment.CENTER)
			, new ExcelHeader("mcateNm", "중분류", HorizontalAlignment.CENTER)
			, new ExcelHeader("scateNm", "소분류", HorizontalAlignment.CENTER)
			, new ExcelHeader("tcNm", "TC명", HorizontalAlignment.CENTER)
			, new ExcelHeader("tcPreEnvir", "사전환경", HorizontalAlignment.CENTER)
			, new ExcelHeader("tcInputVal", "테스트 입력값", HorizontalAlignment.CENTER)
			, new ExcelHeader("tcOutputVal", "예상 결과값", HorizontalAlignment.CENTER)
			, new ExcelHeader("normalCriterion", "정상판단기준", HorizontalAlignment.CENTER)
			, new ExcelHeader("beforeResultCd", "사전테스트 결과", "", "B300", HorizontalAlignment.CENTER)
			, new ExcelHeader("bigo", "비고", HorizontalAlignment.CENTER)
			, new ExcelHeader("useYn", "사용여부", "useYnboolean", HorizontalAlignment.CENTER)
		};
		
		return new ExcelDownloadView(new ExcelResource(list, headers),"H/E버킷리스트.xlsx");
	}
	...
	public ExcelDownloadView getTcResultExcel(Model model, HeTcTmplCriteria heTcTmplCriteria){
		List<HeTcResultVo> list  = hdedTcService.selectProjectTcResult(heTcTmplCriteria.getProjectTcSeq());
		ExcelHeader[] headers = new ExcelHeader[] {
