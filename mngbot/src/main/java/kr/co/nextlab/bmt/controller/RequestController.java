...
public class RequestController extends BaseController {
	...
	public void requestList(Model model, String reqId) {
		model.addAttribute("startDt", new SimpleDateFormat("yyyy-MM-dd").format(DateUtils.addMonths(new Date(), -2)));
	...
	public void requestForm(Model model, String reqId) {
		...
		userCriteria.setUserGroupId("G02");
		userCriteria.setUseYn("Y");
		model.addAttribute("mediaUsers", userService.selectUserList(userCriteria));
	}
	...
	public View modReqProgrssStat(Model model, BmtRequestVo bmtRequestVo){
		...
		if(!proStatChk.getProgressStat().equals("D201") && !proStatChk.getProgressStat().equals("D202")){
			model.addAttribute("save",save);
