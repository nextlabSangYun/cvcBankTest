...
public class QaProjectController extends BaseController {
		...
		try {
			...
			if (!save) model.addAttribute("message", "저장 실패");
				...
				if (qaProjectVo.getAddPartiArray().length > 0) {
					// 등록된 측정 모델리스트 조회
					qaProjectVo.setModelList(qaProjectService.selectQaProjectModelList(qaProjectVo.getPid()));
					// 메시지 알림 등록
					MessageVo messageVo = messageComponent.setMessageCreateQaProject(qaProjectVo, super.getAppId(), super.getUser());
					messageComponent.sendMessage(messageVo, qaProjectVo.getAddPartiArray());
					
					// 메일 발송
					mailComponent.sendMailUser("/mail/qa/newProject", qaProjectVo.getAddPartiArray(), "새 프로젝트 구성원 등록 알림", JsonUtil.getJsonStrFromObject(qaProjectVo));
				}
			}
		} catch (Exception e) {
			if (e.getCause() != null) model.addAttribute("message", "저장 실패");
	 ...
	 * @return 측정모델 버전 정보
	 */
	@RequestMapping("/qa/project/selectQaProjectModelDetailList")
	public View selectQaProjectModelDetailList(Model model, String pid) {
		model.addAttribute("modelList", qaProjectService.selectQaProjectModelDetailList(pid));
		return new MappingJackson2JsonView();
	}
}